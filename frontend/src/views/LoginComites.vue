      ...
      <div class="col-10 col-sm-6 col-md-4 col-lg-4 items-center flex q-my-lg">
        <q-card class="my-card shadow-8 full-width">
          ...
          <q-card-section class="text-center">
            <div class="text-h5 text-weight-bold">ACCESO COMITÉS</div>
          </q-card-section>
          <q-separator />
          ...
          <q-card-section class="q-px-lg q-pb-lg">
            ...
            <div v-if="state === 'DOC_VERIFIED'" class="text-center">
              <div class="text-subtitle2 q-mb-lg">
                Para acceder al módulo de comités, genera un código de
...
async function sendCode() {
  ...
  clearTimers();
  try {
    const res = await post("/comites/access/send-code", { email: useStore.email });
...
async function verifyCode() {
  ...
  try {
    const res = await post("/comites/access/verify-code", { email: useStore.email, code });
    useStore.token = res.token;
    if (res.instructor) useStore.instructorData = res.instructor;
    clearTimers();
    state.value = "CODE_VERIFIED";
    notifySuccessRequest("Acceso a comités concedido");
    setTimeout(() => router.push("/home/comites"), 3000);
  } catch (err) {
    const msg =
