<template>
  <div>
    ...
    >
      ...
      El catálogo puede estar desactualizado. Última carga:
      ...
      <div class="col-12 col-md-9">
        ...
        <div v-if="!hasSearched && !loading" class="text-center q-pa-xl">
          ...
          <div class="text-grey-4 q-mt-xs" style="font-size: 14px">
            Usa el buscador o aplica un filtro para ver los cursos
          </div>
        </div>

        <div
          v-else-if="hasSearched && cursosFiltrados.length === 0 && !loading"
        ...
        <div style="position: relative; min-height: 100px">
          <div class="column q-gutter-sm">
            <div
              v-for="course in cursosPagina"
              :key="course._id"
              class="col-12"
            >
              <CourseCard :course="course" :loading-id="loadingCurso" @select="openDialog" />
            </div>
          </div>
...
import DialogCursoDetalle from "./DialogCursoDetalle.vue";
...
const tieneAlgunFiltro = computed(() => {
  const hayCheckbox = FILTER_CONFIG.filter((f) => f.type === "checkbox").some(
    (f) => (activeFilters.value[f.field] || []).length > 0,
  );
...
const tieneChipsFiltros = computed(
  () => Object.keys(activeFilterChips.value).length > 0,
);

const cursosOrdenados = computed(() => {
  const list = [...cursosFiltrados.value];
  if (sortOrder.value === "horas_asc")
    return list.sort((a, b) => a.prfDuracionMaxima - b.prfDuracionMaxima);
  if (sortOrder.value === "horas_desc")
    return list.sort((a, b) => b.prfDuracionMaxima - a.prfDuracionMaxima);
  if (sortOrder.value === "nombre_asc")
    return list.sort((a, b) => a.prfDenominacion.localeCompare(b.prfDenominacion));
  if (sortOrder.value === "nombre_desc")
    return list.sort((a, b) => b.prfDenominacion.localeCompare(a.prfDenominacion));
  return list;
});

const cursosPagina = computed(() =>
  cursosOrdenados.value.slice(
...
const totalPages = computed(() =>
  Math.ceil(cursosOrdenados.value.length / PAGE_SIZE),
);
...
async function openDialog(course) {
  ...
  try {
    ...
    selectedCourse.value = res?.data || res || course;
  } catch {
