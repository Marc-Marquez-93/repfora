<template>
  <q-dialog :model-value="modelValue" persistent @update:model-value="$emit('update:modelValue', $event)">
    <q-card style="width: 680px; max-width: 96vw; max-height: 92vh" class="flex column">
      ...
      <q-card-section class="bg-green-9 q-px-lg q-py-md" style="flex-shrink: 0">
        <div class="row items-center no-wrap q-gutter-md">
          ...
          <div>
            <div class="text-white text-weight-bold" style="font-size: 17px">ASIGNAR FICHA</div>
            <div class="text-green-2" style="font-size: 12px">Completa los datos administrativos del programa</div>
          </div>
        </div>
      </q-card-section>
      ...
      <div style="flex: 1; overflow-y: auto; min-height: 0">
        <div class="q-pa-md column q-gutter-md">
          ...
          <!-- ── Sección 3: Fechas del instructor (solo lectura) ── -->
          <div>
            <div class="section-title">
              <q-icon name="event" size="16px" class="q-mr-xs" />FECHAS DEL PROGRAMA (instructor)
            </div>
            <div class="row q-col-gutter-sm">
              <div class="col-12 col-sm-6">
                <q-input
...
const opcionesPoblacion  = ref([])
const loadingParametros  = ref(false)

async function cargarParametros() {
  loadingParametros.value = true
  try {
    const [programa, poblacion] = await Promise.all([
      get('/complementary/parametros', { tipo: 'programa'  }),
      get('/complementary/parametros', { tipo: 'poblacion' }),
    ])
    opcionesPrograma.value  = (Array.isArray(programa)  ? programa  : []).map(p => ({ label: p.nombre, value: p.nombre }))
    opcionesPoblacion.value = (Array.isArray(poblacion) ? poblacion : []).map(p => ({ label: p.nombre, value: p.nombre }))
  } catch {}
...
watch(() => props.modelValue, (val) => {
  if (val) {
    ...
    fechaInicio.value          = d?.fechaInicio?.slice(0, 10)             || ''
    fechaFin.value             = d?.fechaFin?.slice(0, 10)                || ''
    fechaInscripcion.value     = d?.fechaInscripcion?.slice(0, 10)        || ''
    fechaMatriculaInicio.value = d?.fechaMatriculaInicio?.slice(0, 10)    || ''
    fechaMatriculaFin.value    = d?.fechaMatriculaFin?.slice(0, 10)       || ''
  }
})
