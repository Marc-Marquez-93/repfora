...
coordinationCtrl.registerCoordination = async (req, res) => {
  const { name, coordinator, modality, email, passapp, namefoldernew, programmers, emailsupervisor } = req.body;
  try {
  ...
  try {
    ...
    await registerAction(
      "COORDINACION",
      {
        ...
        data: { name, coordinator, modality, email, passapp, namefoldernew, programmers, emailsupervisor },
      },
      req.headers.token
    );
